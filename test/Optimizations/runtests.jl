...
@testset "test monotonic loss decrease of optimization with inserting projectors" begin
  ...
  losses_gd = gradient_descent(
    peps, H_local, insert_projectors; stepsize=0.005, num_sweeps=num_sweeps, maxdim=maxdim
  )
  losses_ls = optimize(
    peps, H_local, insert_projectors; num_sweeps=num_sweeps, method="GD", maxdim=maxdim
  )
  losses_lbfgs = optimize(
    peps, H_local, insert_projectors; num_sweeps=num_sweeps, method="LBFGS", maxdim=maxdim
  )
  losses_cg = optimize(
    peps, H_local, insert_projectors; num_sweeps=num_sweeps, method="CG", maxdim=maxdim
  )
  for i in 3:(length(losses_gd) - 1)
    @test losses_gd[i] >= losses_gd[i + 1]
    @test losses_ls[i] >= losses_ls[i + 1]
